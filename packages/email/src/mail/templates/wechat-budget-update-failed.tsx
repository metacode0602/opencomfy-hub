...
interface WechatBudgetUpdateFailedProps extends BaseEmailProps {
  orderId: string
  amount: number
  newBalance: number
  userId: string
...
export function WechatBudgetUpdateFailed({
  orderId,
  amount,
  newBalance,
  userId,
  ...
  locale,
  messages,
}: WechatBudgetUpdateFailedProps) {
  return (
    <EmailLayout locale={locale} messages={messages}>
      <Heading className='font-bold text-2xl text-red-600'>⚠️ 微信支付预算更新失败通知</Heading>

      <Text className='mt-4'>微信支付成功后，更新 LiteLLM 用户预算时发生错误，详情如下：</Text>
      ...
      <Section className='rounded-lg bg-yellow-50 p-4'>
        <Text className='text-gray-700 text-sm'>
          <strong>说明：</strong>
          <br />
          微信支付已成功，数据库余额也已更新，但 LiteLLM 中的用户预算更新失败。
          <br />
...
WechatBudgetUpdateFailed.PreviewProps = {
...
export default WechatBudgetUpdateFailed
