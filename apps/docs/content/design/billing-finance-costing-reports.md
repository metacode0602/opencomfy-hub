...
- [B 端租户全生命周期与客户经理运营](./b2b-tenant-lifecycle-account-management.md)：租户（计费与 CRM 合一）、客户经理与销售归属、充值与消费跟踪、客户动态投影。  
...
同时，运营会通过**算力券**等策略做补贴；预留资源包等预付费形态会跨月摊销。若缺乏统一的「租户级月账单 + 成本归因 + 销售聚合」口径，财务与销售复盘、内控审计会持续争议。
...
4. **销售经营闭环**：在客户经理 / 销售归属（**租户** `tenant_id` 维度）下，输出**每人每月带来的毛利**等经营报表，并与租户生命周期、客户动态中的资金与订单类事件可解释地勾稽。  
...
| 名词 | 定义（本模块中的使用方式） |
|------|---------------------------|
| **租户（Tenant）** | 计费与资源隔离主体，且与 CRM 经营档案**合并为单表 `tenant`**（见 [数据库逻辑模型](./database-schema-structure.md) §1）；月结账单的**最小计费归属**之一；销售归属、毛利聚合均以 **`tenant_id`** 为锚。子账户/项目等如需扩展，挂在租户下另建模，**不再**使用 `commercial_account` / `tenant_binding`。 |
| **计费归属单元（Billing Anchor）** | 月结汇总时挂载消费与归属的实体；与 **`tenant_id`** 对齐（计费与运营同一主键）。 |
| **客户经理分配（AM 分配）** | `account_manager_assignment`：主责 / 协作、生效区间，外键 **`tenant_id`**；**月结销售报表**需使用「账期末日或月内按日切片」的归属策略（见 §6.3）。**租户/账单下钻权限**与运营侧「现任 / 前负责人」时间边界一致（见 §8.1）。 |
...
| **销售毛利（Sales Gross Profit）** | 归集到某销售（或其负责 **`tenant_id`** 列表）在账期内的：**租户收入 − 租户成本**（见 §7.1）。 |
...
| **销售 / 客户经理** | 维护**租户**维度的 AM 归属（交接记录准确）；查看本人及团队授权范围内的毛利与收入报表；不直接改计量事实。 |
...
- **幂等**：同一 `(tenant_id, billing_period, line_type, source_id)` 不可重复入账。  
...
- **主指标**：选定归属策略下的「单人毛利」；  
- **辅助查询**：交接前后对比、未归属清单（无有效 assignment 的租户）。
...
\[
\text{销售毛利} = \sum_{\text{归属该销售的租户}} (\text{租户收入} - \text{租户成本})
\]
...
1. **销售毛利月报**：列表 + 导出；下钻租户 → 产品线 → 账单行 → 取用的条款版本号；**前负责人**下钻时按 §6.3 与 [B2B §8.5.1](./b2b-tenant-lifecycle-account-management.md) 过滤交接后的明细与导出列。  
...
- **现任**：可查看负责租户的**全部历史**账单与消费下钻（在角色允许范围内）。  
...
| 其他文档概念 | 本模块落点 |
|--------------|------------|
| `tenant`（`tenant_id`） | 计费锚点、报表聚合键；与 AM 分配、账单、月结汇总一致 |
| `account_manager_assignment` | 销售毛利归属主数据（`tenant_id`）；`effective_to` 为前负责人租户明细可见上界（对齐 B2B §8.5.1） |
...
| `metering_usage_record` | 计量明细（卡时、存储 GB·月、请求次数等） |
| `billing_invoice` | 租户账期账单头 |
| `billing_invoice_line` | 账单行（产品线、数量、单价、折后额、券抵扣额、source_ref） |
| `cost_accrual_line` | 成本计提行（关联账单行或计量行 + `supplier_deal_terms_version_id`） |
...
erDiagram
  ...
  tenant ||--o{ account_manager_assignment : "销售归属"
...
5. 无 `account_manager_assignment` 覆盖的租户出现在**未归属报表**，不进入销售排名直至补齐或标记例外。  
