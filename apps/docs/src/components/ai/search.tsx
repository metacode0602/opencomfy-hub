...
export function AISearchPanelHeader({
  ...
  return (
    <div
      className={cn(
        "bg-fd-secondary text-fd-secondary-foreground sticky top-0 flex items-start gap-2 rounded-xl border shadow-sm",
        className
      )}
      {...props}
    >
    ...
    >
      ...
      {isLoading ? (
        <button
          ...
          onClick={stop}
        >
          <IconLoader2 className="text-fd-muted-foreground size-4 animate-spin" />
          Abort Answer
        </button>
      ) : (
        <button
  ...
  return (
    <>
      ...
      <Presence present={open}>
        <div
          data-state={open ? "open" : "closed"}
          className="bg-fd-overlay data-[state=open]:animate-fd-fade-in data-[state=closed]:animate-fd-fade-out fixed inset-0 z-30 backdrop-blur-xs lg:hidden"
          onClick={() => setOpen(false)}
        />
      </Presence>
      <Presence present={open}>
    ...
    >
      {messages.length === 0 ? (
        <div className="text-fd-muted-foreground/80 flex size-full flex-col items-center justify-center gap-2 text-center text-sm">
          <IconMessageCircle fill="currentColor" stroke="none" />
