...
/** 文档固定模版 UUID（文生图） */
const DEFAULT_TEMPLATE_UUID = "fe9928fde1b4491c9b360dd24aa2b115"
...
/**
 * POST /api/v1/generate/kontext/text2img
 * 代理 LibLib F.1 Kontext 文生图（上游 POST /api/generate/kontext/text2img）
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    console.warn(
      `[kontext text2img] requestId=${requestId} error=invalid_json_body`,
    )
  ...
  if (!parsed.success) {
    console.warn(
      `[kontext text2img] requestId=${requestId} error=validation issues=${JSON.stringify(parsed.error.flatten())}`,
    )
    return NextResponse.json(
      {
        error: "参数校验失败",
  ...
  console.log(
    `[kontext text2img] requestId=${requestId} action=upstream_start templateUuid=${upstreamBody.templateUuid}`,
  )

  const result = await liblibSignedPostJson(
    "/api/generate/kontext/text2img",
    upstreamBody as Record<string, unknown>,
    { requestId, operation: "kontext_text2img" },
  )

  if (!result.ok) {
    console.warn(
      `[kontext text2img] requestId=${requestId} action=upstream_error`,
    )
    return result.response
  }

  console.log(
    `[kontext text2img] requestId=${requestId} action=upstream_ok`,
  )
