...
function resolveLocalPath(relativePath: string): string {
  return resolveModuleStoragePath(
    LOCAL_STORAGE_SUBDIRS.supplierActivityAttachments,
    relativePath,
  )
}

export async function saveLocalSupplierActivityFile(input: {
  supplierId: string
  activityId: string
  fileName: string
  buffer: Buffer
...
export async function readLocalSupplierActivityFile(storageUri: string): Promise<Buffer> {
