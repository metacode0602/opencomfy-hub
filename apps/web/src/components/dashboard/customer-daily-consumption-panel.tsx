...
import type { Customer, DailyConsumption } from '@/lib/data/types'
import { productLineNames } from '@/lib/data/types'

interface CustomerDailyConsumptionPanelProps {
  customer: Customer
}
...
function DailyConsumptionDetailRows({
  ...
  const { data: details = [], isLoading } =
    trpc.crm.customers.listDailyConsumptionDetails.useQuery(
      {
        customerId,
...
export function CustomerDailyConsumptionPanel({ customer }: CustomerDailyConsumptionPanelProps) {
  const [productLine, setProductLine] = useState(ALL_PRODUCT_LINES)
  const [usageMonth, setUsageMonth] = useState(ALL_MONTHS)
  const [expandedKey, setExpandedKey] = useState<string | null>(null)

  const { data: dailyRows = [], isLoading } = trpc.crm.customers.listDailyConsumptions.useQuery({
              ...
              pagination.items.map((row) => {
                ...
                return (
                  <Fragment key={key}>
                    ...
                    {expanded ? (
                      <DailyConsumptionDetailRows customerId={customer.id} row={row} />
                    ) : null}
                  </Fragment>
                )
              })
