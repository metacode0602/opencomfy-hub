...
import {
  ...
  formatDateTime,
  formatRmb,
} from '@/app/[locale]/(protected)/crm/_components/crm-tenant-billing-shared'
...
export function ProjectBillsPanel({ project }: ProjectBillsPanelProps) {
  const { data: rows = [], isLoading } = trpc.crm.projects.listMonthlyBills.useQuery({
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            来自项目关联租户
            {project.platformTenantId ? `（平台租户 ID：${project.platformTenantId}）` : ''}
            ，共 {rows.length} 条
            {isLoading ? '，加载中…' : ''}
          </CardDescription>
        </CardHeader>
      ...
      >
        <DialogContent className="flex max-h-[85vh] min-w-[40vw] max-w-4xl flex-col gap-0 overflow-hidden p-0">
