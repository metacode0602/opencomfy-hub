...
export type ProjectAccountManagerDialogProps = {
  open: boolean
...
export function ProjectAccountManagerDialog({
  open,
  onOpenChange,
  project,
  onSaved,
}: ProjectAccountManagerDialogProps) {
  const [staffId, setStaffId] = useState('')
  const [effectiveFrom, setEffectiveFrom] = useState(todayShanghaiDateString())
  const [remark, setRemark] = useState('')
  const [submitError, setSubmitError] = useState<string | null>(null)

  const { data: assignment, isLoading } = trpc.crm.projects.getAccountManagerAssignment.useQuery(
    { projectId: project?.id ?? '' },
    { enabled: open && !!project?.id },
  )
  const { data: staff = [] } = trpc.crm.staff.listActive.useQuery(undefined, { enabled: open })

  const saveMutation = trpc.crm.projects.changeAccountManager.useMutation({
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {project
              ? `项目「${project.name}」：可指定生效日期（支持补录历史）。当前主责：${
                  assignment?.staffName ?? project.accountManager ?? '未设置'
                }${
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 py-2">
            <div className="grid gap-2">
              <Label htmlFor="project-am-staff">客户经理</Label>
              <Select value={staffId || undefined} onValueChange={setStaffId}>
                <SelectTrigger id="project-am-staff" className="w-full">
            ...
            <div className="grid gap-2">
              <Label htmlFor="project-am-effective">生效日期</Label>
              <Input
                id="project-am-effective"
                type="date"
                value={effectiveFrom}
                onChange={(e) => {
            ...
            <div className="grid gap-2">
              <Label htmlFor="project-am-remark">备注（可选）</Label>
              <Textarea
                id="project-am-remark"
                rows={2}
                value={remark}
