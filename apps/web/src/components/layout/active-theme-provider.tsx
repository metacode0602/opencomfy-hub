 ...
 *
 * docs:
 * https://gongjiyun.com/docs/themes
 */
export function ActiveThemeProvider({ children, initialTheme }: { children: ReactNode; initialTheme?: string }) {
  ...
  useEffect(() => {
    ...
    const expectedThemeClass = `theme-${activeTheme}`
    // Check if the correct theme class is already applied to avoid unnecessary DOM manipulation
    if (!document.body.classList.contains(expectedThemeClass)) {
      Array.from(document.body.classList)
        .filter((className) => className.startsWith('theme-'))
        .forEach((className) => {
          document.body.classList.remove(className)
        })
      document.body.classList.add(expectedThemeClass)
    }
  }, [activeTheme])
