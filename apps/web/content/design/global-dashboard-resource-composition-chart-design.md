...
**版本**：v1.1（2026-05-29）  
**状态**：**已确认 · v1.0 已实施**（§14 Phase 2 批次事件表待 M1）
...
- [global-dashboard-period-analytics.md](./global-dashboard-period-analytics.md) — Period 回放与卡时
...
Period 须区分 **可回放（实体）** 与 **难回放（计划）** 两类数据。

#### 8.2.1 实体桶 — **支持**（与现 `global-period.ts` 同构）

- 将 `[periodStart, periodEnd]` 切为日/小时桶；
- 用 `supplier_device_change_log` 回放每桶末 `lifecycle_status` / `ops_status` / `in_maintenance`；
- 对每桶调用 `classifyDeviceExclusiveBucket`，累计：
  - **方案 P-A（推荐 v1.0）**：**期末截面** — 仅 `periodEnd` 时刻各桶 `gpuCount` / `deviceCount` 作为 Period 主值（与 KPI Period 主值一致）；
  - **方案 P-B（Phase 2）**：**区间卡时** — 对在服池桶（`pool_*`）按 `在线 ∧ ¬维护` 累计 `gpu × bucketHours`；进程态桶（维护/下架实体等）可按 **状态持续时长 × 卡数** 扩展（需逐桶定义在线判定）。

#### 8.2.2 计划虚拟桶 — **受限支持**

| 扇区 | Period v1.0 | Period Phase 2 |
|------|-------------|----------------|
| `pending_access_pipeline` | **期末截面**：`periodEnd` 时刻批次 `planned − touched` 汇总 | 按 **批次状态变更历史** 或日快照表回放每桶缺口 |
| `retiring_pipeline` | 同上 | 同上 |

**依据**：[supplier-overview-scenarios-from-zero.md §2.1.5](./supplier-overview-scenarios-from-zero.md) 已定 — Period 的 `device_pending_access` **暂不**叠加计划管道吞吐；创建批次 **不会** 在 change_log replay 中产生「进入待接入」事件。

因此：

- Period **可以展示** 资源构成饼图，但 **计划虚拟扇区在 v1.0 仅保证期末截面正确**；
- Period **不能** 在 v1.0 用 change_log 回放「计划缺口随时间的卡时曲线」，除非引入 `onboarding_batch` 历史快照或批次事件表。

#### 8.2.3 Period UI 约定

| 模式 | 饼图主值 | 中心文案 | 副文案 |
|------|----------|----------|--------|
| Snapshot | 卡数/台数截面 | `合计 N 卡` | `其中计划缺口 M 卡` |
| Period v1.0 | **期末** 各桶卡数/台数 | `期末 N 卡` | `净增 …`（实体桶期末 − 期初） |
| Period P-B | 区间卡时 | `合计 N 卡时` | 计划虚拟桶可用「缺口 × 区间时长」近似（**须标注近似**） |

**脚注（Period）**：「计划缺口为期末截面；区间内批次创建/修订不通过变更表回放，缺口变化请结合 Snapshot 或批次列表。」
...
| 计划虚拟桶 Snapshot | ✅ | ✅ |
| 实体互斥桶 Period 期末截面 | ✅ | ✅ |
| 实体桶 Period 区间卡时 | 池桶可沿用现逻辑；进程态桶待扩展 | ✅ |
| 计划虚拟桶 Period 区间内时序 | ❌（仅期末） | ✅（需批次历史） |
...
| **Phase 3** | Period 实体桶净增 + 计划虚拟 **桶末截面**（依赖 §14 M2） |
| **Phase 4** | Period **计划虚拟区间卡时** + DWS `pipeline_gap_*`（§14 M5–M7） |
...
#### 14.7.1 桶末截面（与 v1.0 一致，数据源升级）

对每个 period bucket 末 `t_end`：

```
entity_slices(t_end)  = classify + change_log replay   // 已有
pipeline_slices(t_end) = pipelineGapAt('onboard'|'retire', t_end, filters)
```
...
| **M5** | `global-period` 桶末 + 阶梯卡时 | M2 + Phase 1 |
| **M6** | 回填脚本 + H1/H2 | M1 |
