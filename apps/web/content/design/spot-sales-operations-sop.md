...
3. **顺序不能乱** — 试用发券 →（如有）线下充值到账 → **算力价格审批** → 申请折扣 →（如需）裸金属开通。充值没到账，不要提前申请折扣；**算力价格审批没通过，不要提交折扣工单**。
4. **拿不准就先问** — 任意时刻，若你不确定产品 **能否支持** 客户的具体需求，**先提运营支持工单** 请中台确认，**不要口头承诺** 后再发现做不到。
...
flowchart TB
  ...
  subgraph step4 [第4步 折扣申请 提工单]
    S4a[完成算力价格审批<br/>飞书工单]
    S4[提交折扣工单<br/>附审批结果链接]
    S4b[中台配置优惠价]
    S4c[告知客户正式价格]
  end
  ...
  S2c -->|暂不需要| END1[定期回访]
  S3need -->|有| S3 --> S3b --> S3c --> S4a
  S3need -->|无 客户线上自助充值| S4a
  S4a --> S4 --> S4b --> S4c
  S4c --> S5need{需要裸金属?}
  S5need -->|是| S5 --> S5b --> S5c
  ...
  subgraph anytime [任意时刻 产品能力确认 提工单]
    A0[不确定产品能否<br/>满足客户需求]
    A1[提交运营支持工单]
    A2[中台评估并回复]
    A3[转告客户<br/>再决定下一步]
    A0 --> A1 --> A2 --> A3
...
| **4** | 申请折扣/协议价 | **是**（先算力价格审批，再提折扣工单） | 先完成算力价格审批、再填折扣工单（附审批链接）、告知客户价格 | 配置优惠价 |
| **5** | 裸金属资源开通 | **是**（需要裸金属时） | 确认需求、填工单、帮客户验收 | 后台开通资源 |
| **任意** | 产品能力确认 | **是** | 描述客户需求、提运营支持工单、转告客户结论 | 评估是否支持、说明条件与限制 |



> **说明**：客户如果 **只用闲时算力、不用裸金属**，可以不做第 5 步。第 3 步只有 **对公转账等线下付款** 时才需要。**产品能力确认** 可在 **任意阶段** 发起，不限于上表某一步；建议在 **向客户承诺前** 先提单确认。
...
| **中台运营** | 工单审批通过后，在平台后台发券、入账、配折扣、开通资源；收到 **运营支持工单** 时，评估产品能否满足客户需求并回复 |
...
> **一句话**：你不确定「我们能不能做」时，**提运营支持工单**，让中台帮你答；**确认后再跟客户承诺**。

### 3.1 什么时候提

**任意时刻** 都可以提，常见情况包括：

- 客户询价阶段，问某 **机房地区、显卡型号、数量** 是否有货、能否开通；
- 试用或洽谈中，客户提出 **特殊使用场景**（如特定框架、镜像、网络要求），你不确定平台是否支持；
- 准备提折扣或裸金属工单前，对 **资源库存、交付周期、技术限制** 心里没底；
- 客户把竞品能力或合同条款拿来对比，你需要 **核实我方是否对等支持**。

**原则**：拿不准就提单，**宁可多问一次，不要先答应后打脸**。

### 3.2 工单里要填什么

| 填写项 | 说明 |
|--------|------|
| 客户名称 | 已有档案则与档案一致；询价阶段可填「待建档」 |
| 客户需求描述 | **尽量写客户原话**，并补充你的理解 |
| 涉及产品 | 闲时算力（Spot）/ 裸金属 / 两者都要 |
| 机房地区、显卡型号、数量 | 客户已明确的务必写清；未定的写「待确认」 |
| 期望交付或答复时间 | 客户有无 deadline，便于中台排优先级 |
| 你的联系方式 | 飞书或手机，方便中台追问细节 |

**你不需要懂技术**，把客户问题和已知条件 **如实写进工单** 即可；细节不清楚可以写「待与客户确认」，并在工单备注里 @ 中台协助。
...
sequenceDiagram
  ...
  中台->>销售: 回复是否支持、条件与限制
  销售->>客户: 转告结论
  销售->>销售: 可行则进入对应步骤<br/>不可行则说明或调整方案
```

| 顺序 | 你做什么 | 建议时间 |
| :---: | :--- | :--- |
| 1 | 提交工单，保存 **工单编号** 到客户跟进记录 | 当天 |
| 2 | 中台回复后， **整理成客户能听懂的话** 转告客户 | 收到回复当天 |
| 3 | **支持** → 按结论推进第 1～5 步中对应环节；**不支持或有限制** → 与客户沟通替代方案或暂缓 | — |
| 4 | 把工单号、结论、客户反馈记入跟进记录 | — |

**转告客户的话术参考**：

> 王总，您提的 XX 需求我已经跟内部运营核实过了。目前的情况是：……（支持 / 部分支持 / 暂不支持及原因）。如果可行，咱们下一步可以……；如果有其他想法您也可以跟我说，我再帮您协调。

### 3.4 和五步主流程的关系

| 要点 | 说明 |
|------|------|
| **不改变主流程顺序** | 本工单只解决「能不能做」，不替代算力券、充值、折扣、上架等工单 |
| **可在第 1 步之前提** | 客户还在询价、尚未注册时，就可以先确认能力 |
| **可在第 2～5 步之间提** | 试用或签约过程中发现新需求，随时可提 |
| **与第 5 步的区别** | 第 5 步是 **已确认要做** 后的资源开通；本步是 **还没确认能不能做** 的前置咨询 |

### 3.5 怎么确认「问清楚了」

- 中台已明确回复：**支持 / 部分支持 / 暂不支持**，以及条件、限制或预计等待时间；
- 你已把结论 **转告客户**，客户知晓并认可下一步；
- 工单号和结论已记入客户跟进记录。
...
- **算力价格审批已通过** — 须先单独提交 **算力价格审批** 飞书工单，待审批通过后再提折扣工单；
- 若有 **线下充值**：必须等第 3 步 **入账完成** 后再提折扣工单；
- 若客户 **线上自助充值**：确认余额充足后再提。

> **注意**：算力价格审批与折扣工单是 **两个独立的飞书工单**。价格审批解决「能不能给这个价」；折扣工单用于通知中台在后台 **实际配置** 优惠价。未附价格审批结果链接的折扣工单，可能被退回。

### 7.3 工单里要填什么

#### 第一步：算力价格审批（飞书工单）

按公司算力价格审批模板填写，内容须与后续折扣工单 **一致**，主要包括：
...
**权限提示**：折扣幅度若超出你的授权，算力价格审批工单会自动流转给 **销售主管** 审批，你无需额外操作，但可主动跟进进度。

审批通过后，请 **保存该工单的链接**（飞书工单详情页 URL），供下一步使用。
...
| 申请的优惠价或折扣 | 与算力价格审批一致 |
| 优惠有效期 | 与算力价格审批一致 |
| **算力价格审批结果链接** | **必填** — 粘贴已通过的价格审批飞书工单链接 |
...
| 顺序 | 动作 |
| :---: | :--- |
| 1 | 提交 **算力价格审批** 飞书工单，跟进至审批通过 |
| 2 | 保存价格审批工单的 **链接** |
| 3 | 提交 **折扣申请** 飞书工单，在工单中 **附上价格审批结果链接** |
| 4 | 折扣工单审批通过后，中台会配置优惠价并通知你 |
| 5 | **口头或书面告知客户正式价格及有效期** |
| 6 | 若客户还需要裸金属，进入第 5 步 |
...
| 场景 | 走哪些步骤 |
| :------| :------------|
| 不确定产品能否满足客户需求 | **任意时刻** 提运营支持工单（见第 3 节）；确认可行后再走对应步骤 |
| 新客户，先试用 | 1 → 2 |
| 试用满意，对公转账签约 | 1 → 2 → 3 → 4 |
...
| 产品能力确认（运营支持） | 一般 1 个工作日内 | 飞书联系中台运营；紧急需求在工单中注明 deadline |
| 算力券发放 | 审批通过后 1 个工作日 | 联系中台运营询问进度 |
| 线下充值入账 | 财务确认后 1～2 个工作日 | 确认凭证是否清晰、是否已到账 |
| 算力价格审批 | 按公司审批 SLA | 跟进审批进度；涉及权限则联系主管 |
| 折扣配置 | 价格审批通过且折扣工单审批通过后 1 个工作日 | 确认是否已附价格审批链接；联系中台；涉及权限则联系主管 |
...
| 步骤 | 建议记录内容 |
|------|--------------|
| 产品能力确认 | 工单号、客户问题摘要、中台结论（支持/不支持/条件）、是否已转告客户 |
| 发算力券 | 工单号、面额、有效期、客户是否已查收 |
| 线下充值 | 工单号、金额、到账日、客户是否确认余额 |
| 折扣 | 算力价格审批工单链接、折扣工单号、批准的价格、有效期、是否已告知客户 |
| 裸金属 | 工单号、机房/卡型/数量、开通日、客户是否验收通过 |

**格式示例**：`[产品能力] 飞书工单 20240610002，客户问华北 A800 8 卡，中台回复有货约 3 天可开通，客户 6/13 已知晓`
...
| v1.2 | 2026-06-11 | 第 4 步折扣申请增加前置要求：须先完成算力价格审批（飞书工单），提交折扣工单时须附审批结果链接 |
| v1.3 | 2026-06-16 | 新增「任意时刻：产品能力确认」流程；销售不确定产品能否支持客户需求时可提运营支持工单请中台评估 |
