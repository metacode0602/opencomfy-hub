...
| 目标 | 验收 |
|------|------|
| G1 | 区间 `[periodStart, periodEnd]` 内，每个 **实体互斥扇区** 可算 `cardHours`、`machineHours` | 与分桶规则一致，桶内可闭合 |
| G2 | `pending_access_pipeline`、`retiring_pipeline` 可算区间 **计划占位** 卡时/台时 | 缺口随批次事件阶梯变化 |
| G3 | **期末截面** `gpuCount` 与卡时主值可同时提供 | 卡时为主、期末为副/净增 |
| G4 | 与 Snapshot 互斥构成 **同一套扇区 key** | UI 仅切换单位与副文案 |
| G5 | 主数据与变更表职责 **不交叉** | 符合 D1/D2 |
...
4. 日末截面字段 `lifecycleStatus` / `opsStatus` / `inMaintenance` 取 **当日最后一段** 末态。
...
5. 排除 `lifecycle=退订` / `ops=已退订` 的设备（与 Snapshot 分母一致）。
...
| PC-T4 | 仅创建批次 planned=32，无设备 | `pending_access_pipeline` 卡时 = 32×|τ|；实体池=0 |
